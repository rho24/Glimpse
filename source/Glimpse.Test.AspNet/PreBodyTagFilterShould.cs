...
namespace Glimpse.Test.AspNet
{
    public class PreBodyTagFilterShould
    {
        ...
        private void DoHaveLeftCompressedContentUntouched(string html, int bufferLength)
        {
            ...
            using (var decompressedOutputStream = new MemoryStream())
            {
                ...
                // FAILS HERE ON SECOND BYTE
                Assert.True(StreamEquals(compressedHtmlStream, outputStream));

                //outputStream.Position = 0;
                //using (var decompresser2 = new GZipStream(outputStream, CompressionMode.Decompress, true))
                //{
                //    decompresser2.CopyTo(decompressedOutputStream);
                //}

                //Assert.True(StreamEquals(htmlStream, decompressedOutputStream));

                //decompressedOutputStream.Position = 0;
                //var outputHtml = new StreamReader(decompressedOutputStream, Encoding.UTF8).ReadToEnd();
                //htmlStream.Position = 0;
                //var reEncodedHtml = new StreamReader(htmlStream, Encoding.UTF8).ReadToEnd();

                //Assert.Equal(reEncodedHtml, outputHtml);
            }
        }
